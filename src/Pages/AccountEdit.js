...
//scripts
import {UserPatch, UserDelete, UserInfo, CurrentUser} from '../Axios/UsersController'
//partials
...
export default function AccountEdit (){
	...
	function EmailPatch (){	UserPatch({email: email}); };
	function SexPatch () { UserPatch({sex: sex})};
	function BirthPatch () { UserPatch({birth_year: birth})};
			...
			<div>
				<Content>
					<Section>
						...
						<FormSection>
							<FormSection>
								<RadioButton type="radio" name="sex" value="男性" onClick={() => setSex(true)}/><p>男性</p>
								<RadioButton type="radio" name="sex" value="女性"  onClick={() => setSex(false)}/><p>女性</p>
								<RadioButton type="radio" name="sex" value="その他" onClick={() => setSex(null)}/><p>その他</p>
							</FormSection>
					...
					<Section className="margin-none">
						...
						<FormSection>
							<Input type="text" value={birth} placeholder={'例)'+year} onChange={e => setBirth(e.target.value)}/>
				...
				<Content>
					...
					<Section className="margin-none">
						<h2>アカウント削除</h2>
						<AlertBtn value="削除" type="button" onClick={()=>UserDeleteConfirm()}/>
					</Section>
				</Content>
			</div>
		</FlexJustify>
