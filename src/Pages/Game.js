...
class Game extends React.Component{
	...
	randomJPString = async num =>{
		...
		const wait = (sec) => {
		 ...
		 const element=(<AnswerForm>
		 					<AnswerInput type="text" onChange={e => this.setState({answer:e.target.value})}/>
		 					<AnswerBtn type="button"  value="answer" onClick={()=>this.AnswerCheck(this.state.count)}/>
		 				</AnswerForm>
		 				)
		 try{
	...
	render(){
		return(
			...
			<Container>
				<iframe className="pc" src="https://rcm-fe.amazon-adsystem.com/e/cm?o=9&p=12&l=ur1&category=foodbeverage&f=ifr&linkID=7f4f1565077bd51263df2d0b4042a73f&t=rewordweb-22&tracking_id=rewordweb-22" width="300" height="250" scrolling="no" border="0" marginWidth="0" style={{border:'none', margin: '0 30px',marginTop: '150px'}} frameBorder="0"></iframe>
				<ContentEx>	
					<h3>Reword</h3>
					...
					<CountUI id="count">
						<CalcBtn type='button' value="+" id ="increment" onClick={this.calcInclement}/>
						<CountNum>{this.state.count}</CountNum>
						<CalcBtn type='button' value="-" id ="decrement" onClick={this.calcDeclement}/>
					</CountUI>
					<StartBtn id='startbtn'type='button' value="START" onClick={()=>this.randomJPString(this.state.count)}/>
				</ContentEx>
				<iframe className="pc" src="https://rcm-fe.amazon-adsystem.com/e/cm?o=9&p=12&l=ur1&category=foodbeverage&f=ifr&linkID=7f4f1565077bd51263df2d0b4042a73f&t=rewordweb-22&tracking_id=rewordweb-22" width="300" height="250" scrolling="no" border="0" marginWidth="0" style={{border:'none', margin: '0 30px',marginTop: '150px'}} frameBorder="0"></iframe>
			</Container>
			<Content>
				<Section className="margin-none">
					<h2>遊び方</h2>
					<p>1. ランダムな文字列が提示されます。その文字列を記憶しましょう。</p>
					<p>例) リワード</p>
					<p>2. 記憶した文字列を、反対から読み返しましょう。(メモなどはしないでください)</p>
					<p>例) ドーワリ</p>
					<p>提示された文字列を逆から読み返したものをAnswerの中に回答しましょう。</p>
					<p>例) 回答はドーワリ</p>
				</Section>
			</Content>
			</>
